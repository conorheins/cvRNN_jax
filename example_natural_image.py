...
def main(seed, visualize_dynamics=False, ensemble_size=1):
    ...
    labels_gt = data['lb'].astype(np.int32)  # Ground truth labels
    labels_flat = labels_gt.flatten()

    Nr, Nc = im.shape
    N = Nr * Nc

    seed_specific_dir = os.path.join(output_dir, f'example_natural_image_seed_{seed}')
    ...
    plt.axis('off')
    plt.savefig(os.path.join(seed_specific_dir, 'original.png'), bbox_inches='tight')
    plt.close()
